pipeline {
    ...
    stages {
        stage('Checkout Code') {
            steps {
                git branch: 'main', url: 'git@github.com:DeepikaCAshok/Devops.git'
            }
        }
    ...
    post {
        success {
            ...
            echo "Email notification: Build succeeded! To: deepikaca238@gmail.com"
        }
        failure {
            echo "❌ Build failed for ${APP_NAME}"
            echo "Email notification: Build ${BUILD_NUMBER} failed! To: deepikaca238@gmail.com"
        }
    }
}
